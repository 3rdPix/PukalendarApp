...
class HomeView(QFrame):
    ...
    def RQ_update_time_infobox(self, layer: int, data: list) -> None:
        ...
        match layer:
            ...
            case 1:
                # data = list[list[alias, timedelta]]
                widget_layer_1 = stack.itemAt(1).widget()
                layout_1: QGridLayout = widget_layer_1.layout()
                courses = data
                row_count = 1
                for each in courses:
                    item_1 = layout_1.itemAtPosition(row_count, 0)
                    item_2 = layout_1.itemAtPosition(row_count, 1)
                    if item_1 is not None and item_2 is not None:
                        ...
                        layout_1.addWidget(label_alias, row_count, 0, 1, 1,
                                    alignment=Qt.AlignmentFlag.AlignLeft)
                        layout_1.addWidget(label_time, row_count, 1, 1, 2,
                                    alignment=Qt.AlignmentFlag.AlignLeft)
                    row_count += 1
